...
//自己添加程序，若不对，可修改，发送两个字节
void comSendHalfword(COM_PORT_E _ucPort, uint16_t _ucByte)
{
	uint8_t temp_h, temp_l;
	
	/* 取出高八位 */
	temp_h = (_ucByte&0XFF00)>>8;
	/* 取出低八位 */
	temp_l = _ucByte&0XFF;
	
	
	comSendBuf(_ucPort, &temp_h, 1);
	comSendBuf(_ucPort, &temp_l, 1);

}
void comSendword(COM_PORT_E _ucPort, uint32_t _ucByte)
{
	uint16_t temp_h, temp_l;
	
	/* 取出高十六位 */
	temp_h = (_ucByte&0XFFFF0000)>>16;
	/* 取出低十六位 */
	temp_l = _ucByte&0XFFFF;
	
	comSendHalfword(_ucPort, temp_h);
	comSendHalfword(_ucPort, temp_l);
//	comSendBuf(_ucPort, &temp_h, 1);
//	comSendBuf(_ucPort, &temp_l, 1);
...
//	
//	/* 取出高八位 */
//	temp_a = (_ucByte&0XFF000000)>>24;
//	/* 取出低八位 */
//	temp_b = (_ucByte&0X00FF0000)>>16;
//	temp_c = (_ucByte&0X0000FF00)>>8;
//	temp_d = _ucByte&0XFF;
//	
//	
//	comSendBuf(_ucPort, &temp_a, 1);
//	comSendBuf(_ucPort, &temp_b, 1);
//	comSendBuf(_ucPort, &temp_c, 1);
//	comSendBuf(_ucPort, &temp_d, 1);
...
//	GPIO_Init(GPIOG, &GPIO_InitStructure);
	...
	USART_InitStructure.USART_StopBits = USART_StopBits_1;
	USART_InitStructure.USART_Parity = USART_Parity_No ;
	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;	/* 选择硬件流控 */
	USART_InitStructure.USART_Mode = USART_Mode_Rx | USART_Mode_Tx;
...
static void UartIRQ1(UART_T *_pUart)
{
	u8 Res;
//	u8 clr=0;
	/* 处理接收中断  */
...
//	if(USART_GetFlagStatus(USART3,USART_FLAG_IDLE)!=Bit_RESET)
//	{					                
//		clear=USART3->SR;                
//		clear=USART3->DR;							        
//		RxCounter=0;                
